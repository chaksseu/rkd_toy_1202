...
WANDB_NAME=f"1216_lr1e4_n32_H_b{BATCH_SIZE}_T100_ddim_30_50_steps_teacher_init_rkdW{W_RKD}_invW{W_INV}_invinvW{W_INVINV}_fidW{W_FID}_sameW{W_SAME}_x0_pred_rkd_with_teacher_x0_inv_only_x0_no_norm-jit"
...
CONFIG = {
    ...
    # "student_init_ckpt": "",                     
    "student_init_ckpt": f"ckpt_teacher_B1024_N65536_T100_step1000000.pt",          
    ...
    # wandb
    "use_wandb": True,
    "wandb_project": "RKD-DKDM-1216-H-NO-NORM-JIT-TEACHER_INIT",
...
def colors_from_position(pos: np.ndarray, alpha: float = 0.85):
    ...
    # 점수 계산: (x - y) -> 대각선 방향 그라데이션
    # 좌상단(Blue/Purple) -> 우하단(Red)
    scores = x - y
...
def plot_triplet_scatter_colored(
    ...
    colors: np.ndarray,       # [수정] noise_ids 대신 계산된 colors 배열 (B, 4) 받음
    out_path: Path,
    ...
    # [수정] 내부에서 colors_from_noise_ids 호출하던 부분 삭제
    # colors = colors_from_noise_ids(...)  <-- 삭제
...
def train_student_uniform_xt(cfg: Dict):
    ...
    ddim_sample = make_ddim_like_sample(train_sched, device, cfg["T"])
    ...
    for step_i in range(1, total_steps + 1):
        ...
        xt_S_seq = sample_ddim_student(
            model=student, sample_scheduler=ddim_sample, z=z, 
            device=device, sample_steps=ddim_steps,
            eta=float(cfg.get("ddim_eta", 0.0)), t_sel=t_sel,
        )
        ...
        S_inv_z_seq = sample_ddim_inv_student(
            model=student, sample_scheduler=ddim_sample, x0=x0_batch, 
            device=device, sample_steps=ddim_steps,
       ...
       ##################  RKD LOSSES ##################
        ...
        if (step_i % cfg["vis_interval_epochs"] == 0) or (step_i == total_steps):
            with torch.no_grad():
                ...
                # 여기서 구한 vis_colors는 z_vis의 인덱스와 매핑됨
                ...
                seq_S, _  = collect_xt_seq_ddim(
                    student.eval(), ddim_sample, z_vis, 
                    t_stop=0, return_ts=True, device=device, 
                    sample_steps=int(cfg["rkd_ddim_steps_to_t"])
                )
                ...
                # [STEP 5] Plotting (계산된 vis_colors 전달)
                stride = 1
                ...
                _ = save_and_log_xt_triplets_for_all_t(
                    ...
                    ts          = ts_s,
                    colors      = vis_colors,   # [수정] Teacher x0 기반 색상 전달
                    out_dir     = out_dir,
                    step_i      = step_i,
                    subdir_name = "xt_triplets",
        ...
        #         # [추가] z_vis를 CPU로 가져와서 초기 위치 기반 색상 생성
        ...
        #         seq_S, _  = collect_xt_seq_ddim(student.eval(), ddim_sample, z_vis, t_stop=0, return_ts=True, device=device, sample_steps=int(cfg["rkd_ddim_steps_to_t"]))
        #         seq_S_H   = apply_H_to_seq_per_t(seq_S, ts, H_module, device)
        ...
        #         # [수정] noise_ids 대신 colors 전달
        #         _ = save_and_log_xt_triplets_for_all_t(
        #             teacher_seq = seq_T_s_plot,
        #             student_seq = seq_S_s_plot,
        #             studentH_seq= seq_S_H_s_plot,
        #             ts          = ts_s,
        #             colors      = vis_colors,   # [수정] 생성한 색상 전달
        #             out_dir     = out_dir,
        #             step_i      = step_i,
        #             subdir_name = "xt_triplets",
        #             sync_axes   = bool(cfg.get("vis_xt_sync_axes", False)),
        ...
        # 8) 시각화: 랜덤 노이즈에서 학생 모델로 x0 샘플링하여 저장
        if (step_i % cfg["vis_interval_epochs"] == 0) or (step_i == total_steps):
            @torch.no_grad()
